...
# BENCODER v1.6 - Shen Cheng-Da (cdsheen at gmail.com)
# Project home: http://bencoder.urdada.net/

$title = 'BENCODER v1.6 - Encode your PHP script using bcompiler';
...
for($i = 1;$i < $_SERVER['argc'];$i++) {
	...
	case '-t':
		if(!function_exists('bcompiler_set_filename_handler'))
			bencoder_error('please upgrade your bcompiler extension to support -t option');
		$basename = true;
		break;
...
if($srcdir == '') {
	...
	if($numfiles > 0)
		bencoder_error("You can not encode files and specify `-s DIR' at the same times");
	if($outdir == '')
		bencoder_error("You should use `-o DIR' to specify the output directory");
}
...
if($outdir != '') {
	if(file_exists($outdir)) {
		...
		if(!$force_overwrite)
			bencoder_error("The directory [$outdir] already exists, use -f option to force overwriting");
	}
			...
			if($recursive) {
				if(!file_exists($outpath)) {
					if(mkdir($outpath, 0755))
						...
						bencoder_show_verbose("  mkdir: $outpath (failed)");
				}
...
function bcompiler_encode_file($infile, $outfile) {
	global $basename, $compress;
	$fh = @fopen($outfile, 'w');
    if(!$fh) {
        bencoder_show_verbose(" failed: $outfile (un-writable dir)");
        return(0);
    }
	if($basename) {
	...
	if($compress) {
		...
		bzclose($bzfh);
		bencoder_show_verbose("encoded: $outfile (compressed)");
   	}
	else
